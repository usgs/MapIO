...
class Cities(object):
    ...
    def __init__(self, dataframe):
        ...
        Returns:
            dataframe (pandas dataframe):
                where each row represents a city.
                Columns include:
    ...
    def loadFromGeoNames(cls, cityfile=None):
        ...
        Args:
            cityfile (str):
                Path to cities1000.txt file from GeoNames, or None
    ...
    def sortByColumns(self, columns, ascending=True):
        ...
        Args:
            columns (str or :tuple:'list'):
                String name or list of names of any of the columns that are in the
                internal dataframe.  See getColumns().
                Only the required set of columns (see __init__ method)
                are guaranteed to be present, but subclasses of Cities may add more.
            ascending (bool):
                indicating which direction values should be sorted.
        Raises:
            DataSetException:
    ...
    def limitByPopulation(self, pop, minpop=0):
        ...
        if "pop" not in self._dataframe.columns:
            raise DataSetException(
                "Cities instance does not contain population information"
            )
        if minpop >= pop:
            raise DataSetException(
    ...
    def limitByName(self, cityname):
        ...
        Returns:
            Cities instance containing cities
            with names that match the input name/regular expression.
        """

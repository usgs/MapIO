...
def sub2ind(shape, subtpl):
    ...
    Args:
        shape(Tuple): indicating size of 2D array.
        subtpl(Tuple): of (possibly) numpy arrays of row,col values.
    Returns:
        1D array of indices.
    """
    if len(shape) != 2 or len(shape) != len(subtpl):
        raise IndexError(
            "Input size and subscripts must have length 2 and " "be equal in length"
        )
...
def createSampleGrid(M, N):
    ...
    Returns:
       GMTGrid(class):
            object where data values are an NxN array of values from 0
...
class GMTGrid(Grid2D):
    ...
    def getNetCDFHeader(cls, filename):
        ...
        Returns:
            GeoDict(geodict):
                ...
                grid inside NetCDF file.
            xvar(array):
                specifying X coordinates of data columns
            yvar(array):
                specifying Y coordinates of data rows
        """
        cdf = netcdf.netcdf_file(filename)
        geodict = {}
        xvarname = None
        ...
        if xvarname is not None:
            ...
            raise DataSetException(
                "No support for CDF data file with "
    ...
    @classmethod
    def _subsetRegions(
        self, zvar, sampledict, fgeodict, xvar, yvar, firstColumnDuplicated
    ):
        ...
        if xmin == gxmin and xmax == gxmax and ymin == gymin and ymax == gymax:
            ...
            if xmin > xmax:
                ...
                if isScanLine:
                    ...
                    data = np.flipud(
                        indexArray(zvar, (gny, gnx), iuly, ilry + 1, iulx, ilrx + 1)
                    )
    ...
    def save(self, filename, format="netcdf"):
        ...
        if format == "netcdf":
            ...
            f.attrs["history"] = (
                "Created with python " 'GMTGrid.save(%s,format="hdf")' % filename
            )
            ...
            f.write(
                struct.pack(
                    "320s", b"Created with GMTGrid() class, a " b"product of the NEIC."
                )
            )  # command
